...
from transformers.utils import Batch, Model
...
class Trainer:
  ...
  compute_metrics: Callable[[nn.Module], dict[str, Any]]|None = None
      ...
      for batch in self.train_loader:
        ...
        if step_count > 0 and self.eval_steps and step_count % self.eval_steps == 0:
          ...
          if self.compute_metrics:
            metrics = self.compute_metrics(self.model)
            self.metrics.add(step_count, metrics)
          eval_time = timer() - t0
